...
int main() {
    ...
    // --- NOVO DESAFIO: MOVIMENTO DO CAVALO: usando loops aninhados ---
    ...
    // Usamos 'for' pois o número de repetições é fixo e conhecido.
    for (int i = 0; i < 2; i++) {
        printf("Baixo\n");
    }

    // Segundo loop (interno) para o movimento horizontal (1 casa para a esquerda)
    // O loop 'while' executa a ação uma única vez para completar o 'L'.
    int movimento_horizontal = 1;
    while (movimento_horizontal > 0) {
        printf("Esquerda\n");
        movimento_horizontal--; // Decrementa a variável para encerrar o loop.
    }
